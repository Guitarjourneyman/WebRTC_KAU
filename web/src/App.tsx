...
//<kau> const SOCKET_SERVER_URL = 'https://localhost:8000';
 const SOCKET_SERVER_URL = '192.168.0.5:8000'; // Change to your server URL to https from http
...
const App = () => {
	...
	const myIsDisabledVideoRef = useRef<boolean | null>(
		sessionStorage.getItem("isDisabledVideo") === null
			? null
			: sessionStorage.getItem("isDisabledVideo") === "true"
	);
	// 삭제
	...
	const getLocalStream = useCallback(async () => {
		try {
			...
			socketRef.current.on("initial_setting", ({ student }: { student: { studentId: string; isDisabledVideo: boolean } }) => {
				const { studentId, isDisabledVideo } = student;

				myStudentIdRef.current = studentId;
				myIsDisabledVideoRef.current = isDisabledVideo;

				sessionStorage.setItem("studentId", studentId);
				sessionStorage.setItem("isDisabledVideo", isDisabledVideo ? "true" : "false");

				console.log("Assigned initial_setting:", student);
			if (socketRef.current) {
	...
	const createPeerConnection = useCallback((socketID: string, studentId: string, isOfferer: boolean,isDisabledVideo: boolean) => {
		try {
			//<kau> a peer object generated by each peer connection individually
			...
			pc.onicecandidate = (e) => {
				...
				console.log('(3-1)onicecandidate: ',e.candidate);
			...
			if (localStreamRef.current) {
				...
				localStreamRef.current.getTracks().forEach((track) => {
					...
					// <kau> Add the local stream track (audio & video)to the peer connection
...
const toggleVideoSend = (targetSocketId: string) => {
  ...
  // <kau> 1. P1: Off command to P2 via the server
  if (socketRef.current) {
...
// Change the size of my video window 
	...
	useEffect(() => {
		...
		socketRef.current.on(
			...
			(data: { sdp: RTCSessionDescription; answerSendID: string }) => {
				...
				console.log('(2-2)get answer', data.answerSendID);
		...
		socketRef.current.on(
			...
			async (data: { candidate: RTCIceCandidateInit; candidateSendID: string }) => {
				console.log('(4-1)get candidate', data.candidate);
				const pc: RTCPeerConnection = pcsRef.current[data.candidateSendID];
				if (!pc) {
					console.log("Removed candidate: ",data.candidate);
					return;
				}
		...
		// <kau> toggle video
		socketRef.current.on('toggleVideo', (data: { studentId: string; disabledVideo: boolean }) => {
		const myId = myStudentIdRef.current;
		const targetStudent = data.studentId;
		console.log('toggleVideo: myID =', myId, ', targetID =', targetStudent);

		if (targetStudent === myId) {
			const pcEntries = Object.entries(pcsRef.current);

			pcEntries.forEach(([remoteSocketId, pc]) => {
			const senders = pc.getSenders();

			if (data.disabledVideo === true) {
				// Disable both video and audio by replacing tracks with null
				senders.forEach((sender) => {
				if (sender.track && (sender.track.kind === 'video' || sender.track.kind === 'audio')) {
					sender.replaceTrack(null);
					console.log(`[self] ${sender.track.kind} track disabled for ${remoteSocketId}`);
				}
				});

				myIsDisabledVideoRef.current = data.disabledVideo;
				sessionStorage.setItem("isDisabledVideo", String(true));
				console.log('sessionStorage Set', sessionStorage);
			} else {
				// Re-enable both video and audio
				const videoTrack = localStreamRef.current?.getVideoTracks()[0];
				const audioTrack = localStreamRef.current?.getAudioTracks()[0];
				console.log('else ----');
				pc.getTransceivers().forEach(transceiver => {
					if (transceiver.sender) {
						if (transceiver.receiver.track?.kind === 'video' && videoTrack) {
						transceiver.sender.replaceTrack(videoTrack);
						console.log(`[self] video track re-enabled `);
						}
				...
				myIsDisabledVideoRef.current = data.disabledVideo;
				sessionStorage.setItem("isDisabledVideo", String(false));
				console.log('sessionStorage Set', sessionStorage);
			}
			});
		}

		// If the target is someone else
		else {
			console.log(`[other] Updating disabled state for ${targetStudent} → ${data.disabledVideo}`);
			setUsers((prevUsers) =>
			prevUsers.map((user) =>
				user.studentId === targetStudent
				? { ...user, isDisabledVideo: data.disabledVideo }
				: user
			)
			);
